...
%     psd_dir = strcat(SBJ_vars.dirs.import,'raw_psds/');
    psd_dir = strcat(SBJ_vars.dirs.import,fullfile('raw_psds',filesep));
...
%     end
    ...
    for ch_ix = 1:numel(data.label)
        if ~isempty(find(data.label{ch_ix}==fullfile(filesep),1))   % These will mess up paths when saved
            data.label{ch_ix}(data.label{ch_ix}==fullfile(filesep)) = '_';
        end
    end    
